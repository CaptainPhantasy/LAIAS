...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "complexity IN ('simple', 'moderate', 'complex')", name="ck_agents_complexity"
        ),
        sa.CheckConstraint(
            "task_type IN ('research', 'development', 'analysis', 'automation', 'general')",
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "status IN ('pending', 'creating', 'starting', 'running', 'stopped', 'error', 'terminated')",
    ...
    op.add_column(
        "agents", sa.Column("owner_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id"))
    )
    op.add_column(
        "agents", sa.Column("team_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("teams.id"))
    )
