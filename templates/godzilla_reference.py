...
class AgentState(BaseModel):
    ...
    confidence: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Result confidence score"
    )
    ...
    intermediate_results: Dict[str, Any] = Field(
        default_factory=dict, description="Work in progress"
    )
    final_results: Dict[str, Any] = Field(
        default_factory=dict, description="Completed outputs"
    )
...
class LegacyAIPrimeFlow(Flow[AgentState]):
    ...
    def __init__(self, config: Optional[AgentConfig] = None, **kwargs):
        ...
        Args:
            ...
            **kwargs: Passed through to Flow.__init__ (required by @persist())
        """
    ...
    @start()
    async def initialize_execution(self) -> AgentState:
        """
        ...
        NOTE: Do NOT add 'inputs' as a positional argument here.
        When using @persist(), the decorator wraps __init__ and the
        @start() method in a way that strips positional args.
        Instead, pass inputs via kickoff_async(inputs={...}) and
        access them through self.state fields, which CrewAI
        pre-populates from the inputs dict before calling this method.
        ...
        try:
            ...
            # Validate inputs
            if not self._validate_inputs(self.state.inputs):
    ...
    async def analyze_requirements(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            logger.info("Requirements analysis complete", task_id=self.state.task_id)
    ...
    async def handle_error_recovery(self, state: AgentState) -> AgentState:
        ...
        self.state.metadata["retry_count"] = (
            self.state.metadata.get("retry_count", 0) + 1
        )
