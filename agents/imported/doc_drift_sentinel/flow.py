...
class DocDriftSentinelState(BaseModel):
    """Typed state for doc_drift_sentinel Agent."""
...
class DocDriftSentinelFlow(Flow[DocDriftSentinelState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> DocDriftSentinelState:
    ...
    @listen("initialize")
    async def execute(self, state: DocDriftSentinelState) -> DocDriftSentinelState:
        ...
        try:
            ...
            agent = Agent(
                role="doc_drift_sentinel Agent",
                goal="You exist to solve the documentation drift problem: code changes while documentation stays the same,",
                backstory="""You are the doc_drift_sentinel Agent, a specialized documentation guardian responsible for detecting and resolving semantic drift between code and documentation.
...
- Meticulous about details but not""",
                tools=self.tools,
...
- You're preparing for a release and want docs verified
    ...
    @listen("execute")
    async def finalize(self, state: DocDriftSentinelState) -> DocDriftSentinelState:
