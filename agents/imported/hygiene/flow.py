...
class HygieneState(BaseModel):
    """Typed state for HYGIENE REPORT."""
...
class HygieneFlow(Flow[HygieneState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> HygieneState:
    ...
    @listen("initialize")
    async def execute(self, state: HygieneState) -> HygieneState:
        ...
        try:
            ...
            agent = Agent(
                role="HYGIENE REPORT",
                goal="",
                backstory="""You are the HYGIENE REPORT. """,
                tools=self.tools,
    ...
    @listen("execute")
    async def finalize(self, state: HygieneState) -> HygieneState:
