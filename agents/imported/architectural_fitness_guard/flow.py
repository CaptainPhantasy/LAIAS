...
class ArchitecturalFitnessGuardState(BaseModel):
    """Typed state for architectural_fitness_guard Agent."""
...
class ArchitecturalFitnessGuardFlow(Flow[ArchitecturalFitnessGuardState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> ArchitecturalFitnessGuardState:
    ...
    @listen("initialize")
    async def execute(self, state: ArchitecturalFitnessGuardState) -> ArchitecturalFitnessGuardState:
        ...
        try:
            ...
            agent = Agent(
                role="architectural_fitness_guard Agent",
                goal="You exist to maintain the "architectural fitness" of the codebase - the degree to which the code fol",
                backstory="""You are the architectural_fitness_guard Agent, a specialized architect responsible for maintaining architectural integrity and enforcing design principles.
...
- Principled - rules exist for reasons, not arbitrari""",
                tools=self.tools,
...
- You've just merged code and want to validate architectural integrity
    ...
    @listen("execute")
    async def finalize(self, state: ArchitecturalFitnessGuardState) -> ArchitecturalFitnessGuardState:
...
async def main():
    ...
    flow = ArchitecturalFitnessGuardFlow()
