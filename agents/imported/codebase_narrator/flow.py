...
class CodebaseNarratorState(BaseModel):
    """Typed state for codebase_narrator Agent."""
...
class CodebaseNarratorFlow(Flow[CodebaseNarratorState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> CodebaseNarratorState:
    ...
    @listen("initialize")
    async def execute(self, state: CodebaseNarratorState) -> CodebaseNarratorState:
        ...
        try:
            ...
            agent = Agent(
                role="codebase_narrator Agent",
                goal="You exist to solve the lost context problem. Without narrative documentation:
- Future you forget wh",
                backstory="""You are the codebase_narrator Agent, a specialized literary technical writer responsible for crafting narrative prose histories of development work.
...
- You want to maintain a living history of the project
    ...
    @listen("execute")
    async def finalize(self, state: CodebaseNarratorState) -> CodebaseNarratorState:
