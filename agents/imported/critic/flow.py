...
class CriticState(BaseModel):
    """Typed state for CRITIC EVIDENCE REPORT — CYCLE <N>."""
...
class CriticFlow(Flow[CriticState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> CriticState:
    ...
    @listen("initialize")
    async def execute(self, state: CriticState) -> CriticState:
        ...
        try:
            ...
            agent = Agent(
                role="CRITIC EVIDENCE REPORT — CYCLE <N>",
                goal="",
                backstory="""You are the CRITIC EVIDENCE REPORT — CYCLE <N>. """,
                tools=self.tools,
    ...
    @listen("execute")
    async def finalize(self, state: CriticState) -> CriticState:
