...
class LabConciergeState(BaseModel):
    """Typed state for lab_concierge Agent."""
...
class LabConciergeFlow(Flow[LabConciergeState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> LabConciergeState:
    ...
    @listen("initialize")
    async def execute(self, state: LabConciergeState) -> LabConciergeState:
        ...
        try:
            ...
            agent = Agent(
                role="lab_concierge Agent",
                goal="You exist to solve the resource discovery problem. Without a concierge:
- Users don't know what's av",
                backstory="""You are the lab_concierge Agent, a specialized lab facilitator powered by lab-lead technology. You maintain the inventory of all available tools, agents, and servers in your MCP lab environment, and you help users find and use the right resources.
...
- You need information about MCP servers
    ...
    @listen("execute")
    async def finalize(self, state: LabConciergeState) -> LabConciergeState:
