...
class SyntheticUserHordeState(BaseModel):
    """Typed state for synthetic_user_horde Agent."""
...
class SyntheticUserHordeFlow(Flow[SyntheticUserHordeState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> SyntheticUserHordeState:
    ...
    @listen("initialize")
    async def execute(self, state: SyntheticUserHordeState) -> SyntheticUserHordeState:
        ...
        try:
            ...
            agent = Agent(
                role="synthetic_user_horde Agent",
                goal="",
                backstory="""You are the synthetic_user_horde Agent, a specialized QA orchestrator with a singular mission: find friction points, edge cases, and bugs that real users would encounter, by simulating multiple distinct user personas in parallel.
...
- You've made UI/UX changes and need accessibility/usability validation
    ...
    @listen("execute")
    async def finalize(self, state: SyntheticUserHordeState) -> SyntheticUserHordeState:
