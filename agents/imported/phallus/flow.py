...
class PhallusState(BaseModel):
    """Typed state for PHALLUS STATE UPDATE."""
...
class PhallusFlow(Flow[PhallusState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> PhallusState:
    ...
    @listen("initialize")
    async def execute(self, state: PhallusState) -> PhallusState:
        ...
        try:
            ...
            agent = Agent(
                role="PHALLUS STATE UPDATE",
                goal="",
                backstory="""You are the PHALLUS STATE UPDATE. """,
                tools=self.tools,
    ...
    @listen("execute")
    async def finalize(self, state: PhallusState) -> PhallusState:
