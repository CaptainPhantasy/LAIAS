"""
================================================================================
                    PHALLUS INTEGRATION HOOK
## Pre-Task Hook
Before starting any task:
1. Read .phallus/progress.m
================================================================================
...
class PhallusHookTemplateState(BaseModel):
    """Typed state for PHALLUS Integration Hook."""
...
Before starting any task:
1. Read .phallus/progress.md
2. Check current gate status
    ...
    def _initialize_tools(self) -> List:
        ...
        for tool_name in ['FileReadTool', 'SerperDevTool']:
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> PhallusHookTemplateState:
    ...
    @listen("initialize")
    async def execute(self, state: PhallusHookTemplateState) -> PhallusHookTemplateState:
        ...
        try:
            ...
            agent = Agent(
                role="PHALLUS Integration Hook",
                goal="",
                backstory="""You are the PHALLUS Integration Hook. ## Pre-Task Hook
Before starting any task:
1. Read .phallus/progress.md
2. Check current gate status
3. Load master_plan.md context""",
                tools=self.tools,
    ...
    @listen("execute")
    async def finalize(self, state: PhallusHookTemplateState) -> PhallusHookTemplateState:
