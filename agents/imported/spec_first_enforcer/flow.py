...
class SpecFirstEnforcerState(BaseModel):
    """Typed state for spec_first_enforcer Agent."""
...
class SpecFirstEnforcerFlow(Flow[SpecFirstEnforcerState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> SpecFirstEnforcerState:
    ...
    @listen("initialize")
    async def execute(self, state: SpecFirstEnforcerState) -> SpecFirstEnforcerState:
        ...
        try:
            ...
            agent = Agent(
                role="spec_first_enforcer Agent",
                goal="You exist to solve the "code-first, think-later" problem. Without spec-first discipline:
- Features ",
                backstory="""You are the spec_first_enforcer Agent, a specialized methodology enforcer responsible for ensuring specifications exist before code is written.
...
- You need to establish or enforce spec-first workflow
    ...
    @listen("execute")
    async def finalize(self, state: SpecFirstEnforcerState) -> SpecFirstEnforcerState:
