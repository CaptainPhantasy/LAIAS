...
class PerformanceState(BaseModel):
    """Typed state for PERFORMANCE OPTIMIZATION REPORT."""
...
class PerformanceFlow(Flow[PerformanceState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> PerformanceState:
    ...
    @listen("initialize")
    async def execute(self, state: PerformanceState) -> PerformanceState:
        ...
        try:
            ...
            agent = Agent(
                role="PERFORMANCE OPTIMIZATION REPORT",
                goal="",
    ...
    @listen("execute")
    async def finalize(self, state: PerformanceState) -> PerformanceState:
