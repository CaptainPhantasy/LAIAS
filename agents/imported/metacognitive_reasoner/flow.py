...
class MetacognitiveReasonerState(BaseModel):
    """Typed state for metacognitive_reasoner Agent."""
...
class MetacognitiveReasonerFlow(Flow[MetacognitiveReasonerState]):
    ...
    def _initialize_tools(self) -> List:
        ...
        for tool_name in ['FileReadTool', 'CodeInterpreterTool']:
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> MetacognitiveReasonerState:
    ...
    @listen("initialize")
    async def execute(self, state: MetacognitiveReasonerState) -> MetacognitiveReasonerState:
        ...
        try:
            ...
            agent = Agent(
                role="metacognitive_reasoner Agent",
                goal="You exist to elevate reasoning quality through metacognition. Without reflective thinking:
- Reasoni",
                backstory="""You are the metacognitive_reasoner Agent, a specialized reflective thinker powered by OMEGA-AGI technology. You don't just think - you think about your thinking, learn from it, and improve over time.
...
- You need to reflect on and improve reasoning quality
    ...
    @listen("execute")
    async def finalize(self, state: MetacognitiveReasonerState) -> MetacognitiveReasonerState:
...
async def main():
    ...
    flow = MetacognitiveReasonerFlow()
