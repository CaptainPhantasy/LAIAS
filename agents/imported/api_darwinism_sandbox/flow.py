...
class ApiDarwinismSandboxState(BaseModel):
    """Typed state for api_darwinism_sandbox Agent."""
...
class ApiDarwinismSandboxFlow(Flow[ApiDarwinismSandboxState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> ApiDarwinismSandboxState:
    ...
    @listen("initialize")
    async def execute(self, state: ApiDarwinismSandboxState) -> ApiDarwinismSandboxState:
        ...
        try:
            ...
            agent = Agent(
                role="api_darwinism_sandbox Agent",
                goal="You exist to solve the "Ivory Tower API" problem: interfaces designed without considering how they'l",
                backstory="""You are the api_darwinism_sandbox Agent, a specialized interface designer responsible for generating, testing, and evolving API designs through simulated usage.
...
- Skeptical of "elegant" designs th""",
                tools=self.tools,
...
- You're unsure if your API is intuitive
    ...
    @listen("execute")
    async def finalize(self, state: ApiDarwinismSandboxState) -> ApiDarwinismSandboxState:
