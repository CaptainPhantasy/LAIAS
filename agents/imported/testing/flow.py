...
class TestingState(BaseModel):
    """Typed state for TRIPWIRE REPORT."""
...
class TestingFlow(Flow[TestingState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> TestingState:
    ...
    async def execute(self, state: TestingState) -> TestingState:
        ...
        try:
            ...
            agent = Agent(
                role="TRIPWIRE REPORT",
                goal="",
                backstory="""You are the TRIPWIRE REPORT. """,
                tools=self.tools,
    ...
    @listen("execute")
    async def finalize(self, state: TestingState) -> TestingState:
