...
class PreCommitDevilsAdvocateState(BaseModel):
    """Typed state for pre_commit_devils_advocate Agent."""
...
class PreCommitDevilsAdvocateFlow(Flow[PreCommitDevilsAdvocateState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> PreCommitDevilsAdvocateState:
    ...
    @listen("initialize")
    async def execute(self, state: PreCommitDevilsAdvocateState) -> PreCommitDevilsAdvocateState:
        ...
        try:
            ...
            agent = Agent(
                role="pre_commit_devils_advocate Agent",
                goal="You exist to solve the "I wish someone had stopped me" problem. Without adversarial review:
- Bad as",
                backstory="""You are the pre_commit_devils_advocate Agent, a specialized adversarial reviewer responsible for challenging assumptions, identifying risks, and creating constructive friction before code is committed.
...
- You want to prevent regrettable commits
    ...
    @listen("execute")
    async def finalize(self, state: PreCommitDevilsAdvocateState) -> PreCommitDevilsAdvocateState:
...
async def main():
    ...
    flow = PreCommitDevilsAdvocateFlow()
