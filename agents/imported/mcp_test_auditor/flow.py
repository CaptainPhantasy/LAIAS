...
class McpTestAuditorState(BaseModel):
    """Typed state for mcp_test_auditor Agent."""
...
class McpTestAuditorFlow(Flow[McpTestAuditorState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> McpTestAuditorState:
    ...
    @listen("initialize")
    async def execute(self, state: McpTestAuditorState) -> McpTestAuditorState:
        ...
        try:
            ...
            agent = Agent(
                role="mcp_test_auditor Agent",
                goal="You exist to solve the "works on my machine" problem. Without real-world testing:
- Tools fail in pr",
                backstory="""You are the mcp_test_auditor Agent, a specialized quality assurance expert for MCP (Model Context Protocol) servers and tools. You evaluate tools based on how they perform in real-world scenarios, not artificial test cases.
...
- You want to measure actual usability (not synthetic metrics)
    ...
    @listen("execute")
    async def finalize(self, state: McpTestAuditorState) -> McpTestAuditorState:
