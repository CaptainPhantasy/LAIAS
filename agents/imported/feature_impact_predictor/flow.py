...
class FeatureImpactPredictorState(BaseModel):
    """Typed state for feature_impact_predictor Agent."""
...
class FeatureImpactPredictorFlow(Flow[FeatureImpactPredictorState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> FeatureImpactPredictorState:
    ...
    @listen("initialize")
    async def execute(self, state: FeatureImpactPredictorState) -> FeatureImpactPredictorState:
        ...
        try:
            ...
            agent = Agent(
                role="feature_impact_predictor Agent",
                goal="You exist to solve the feature estimation problem. Without proper impact analysis, features:
- Take ",
                backstory="""You are the feature_impact_predictor Agent, a specialized impact analyst responsible for predicting the full scope, risk, and effort required for feature development.
...
- Realistic about effort - you've seen "simple" f""",
                tools=self.tools,
...
- You need to create a feature implementation plan
    ...
    @listen("execute")
    async def finalize(self, state: FeatureImpactPredictorState) -> FeatureImpactPredictorState:
...
async def main():
    ...
    flow = FeatureImpactPredictorFlow()
