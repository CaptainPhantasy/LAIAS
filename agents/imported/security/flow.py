...
class SecurityState(BaseModel):
    """Typed state for SECURITY HARDENING REPORT."""
...
class SecurityFlow(Flow[SecurityState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> SecurityState:
    ...
    @listen("initialize")
    async def execute(self, state: SecurityState) -> SecurityState:
        ...
        try:
            ...
            agent = Agent(
                role="SECURITY HARDENING REPORT",
                goal="",
                backstory="""You are the SECURITY HARDENING REPORT. """,
                tools=self.tools,
    ...
    @listen("execute")
    async def finalize(self, state: SecurityState) -> SecurityState:
