...
class ArchitectureState(BaseModel):
    """Typed state for ARCHITECTURE REPAIR REPORT."""
...
class ArchitectureFlow(Flow[ArchitectureState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> ArchitectureState:
    ...
    async def execute(self, state: ArchitectureState) -> ArchitectureState:
        ...
        try:
            ...
            agent = Agent(
                role="ARCHITECTURE REPAIR REPORT",
                goal="",
                backstory="""You are the ARCHITECTURE REPAIR REPORT. """,
                tools=self.tools,
    ...
    @listen("execute")
    async def finalize(self, state: ArchitectureState) -> ArchitectureState:
