...
class ProjectBiographerState(BaseModel):
    """Typed state for project_biographer Agent."""
...
class ProjectBiographerFlow(Flow[ProjectBiographerState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> ProjectBiographerState:
    ...
    @listen("initialize")
    async def execute(self, state: ProjectBiographerState) -> ProjectBiographerState:
        ...
        try:
            ...
            agent = Agent(
                role="project_biographer Agent",
                goal="You exist to answer questions like:
- "Why is this module structured this way?"
- "What were we thin",
                backstory="""You are the project_biographer Agent, a specialized historian responsible for capturing and narrating the human story of software development.
...
- You need to generate release notes or changelogs with context
    ...
    @listen("execute")
    async def finalize(self, state: ProjectBiographerState) -> ProjectBiographerState:
