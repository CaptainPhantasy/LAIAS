...
class DebtCollectorState(BaseModel):
    """Typed state for debt_collector Agent."""
...
class DebtCollectorFlow(Flow[DebtCollectorState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> DebtCollectorState:
    ...
    @listen("initialize")
    async def execute(self, state: DebtCollectorState) -> DebtCollectorState:
        ...
        try:
            ...
            agent = Agent(
                role="debt_collector Agent",
                goal="You exist to solve the unmanaged technical debt problem. Without tracking, debt accumulates invisibl",
                backstory="""You are the debt_collector Agent, a specialized technical debt accountant responsible for tracking, quantifying, and prioritizing technical debt with compound interest.
...
- Precise in debt calculation - you don't guess, """,
                tools=self.tools,
...
- You want to track debt accumulation over time
    ...
    @listen("execute")
    async def finalize(self, state: DebtCollectorState) -> DebtCollectorState:
