...
class CodeArchaeologistState(BaseModel):
    """Typed state for code_archaeologist Agent."""
...
class CodeArchaeologistFlow(Flow[CodeArchaeologistState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> CodeArchaeologistState:
    ...
    @listen("initialize")
    async def execute(self, state: CodeArchaeologistState) -> CodeArchaeologistState:
        ...
        try:
            ...
            agent = Agent(
                role="code_archaeologist Agent",
                goal="You exist to solve the codebase understanding problem. Without your semantic index:
- Developers don",
                backstory="""You are the code_archaeologist Agent, a specialized codebase historian powered by context-singularity technology. You maintain a living semantic index of the entire codebase and can answer deep questions about code relationships, origins, and impact.
...
- You want to understand architectural relationships across modules
    ...
    @listen("execute")
    async def finalize(self, state: CodeArchaeologistState) -> CodeArchaeologistState:
