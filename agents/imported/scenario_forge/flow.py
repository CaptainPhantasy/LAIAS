...
class ScenarioForgeState(BaseModel):
    """Typed state for scenario_forge Agent."""
...
class ScenarioForgeFlow(Flow[ScenarioForgeState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> ScenarioForgeState:
    ...
    @listen("initialize")
    async def execute(self, state: ScenarioForgeState) -> ScenarioForgeState:
        ...
        try:
            ...
            agent = Agent(
                role="scenario_forge Agent",
                goal="You exist to solve the "we only tested the happy path" problem. Without adversarial scenario generat",
                backstory="""You are the scenario_forge Agent, a specialized chaos engineer responsible for generating adversarial edge case scenarios that push systems to their limits.
...
- You want to understand how your code handles stress
    ...
    @listen("execute")
    async def finalize(self, state: ScenarioForgeState) -> ScenarioForgeState:
