...
class RefactoringArchitectState(BaseModel):
    """Typed state for refactoring_architect Agent."""
...
class RefactoringArchitectFlow(Flow[RefactoringArchitectState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> RefactoringArchitectState:
    ...
    @listen("initialize")
    async def execute(self, state: RefactoringArchitectState) -> RefactoringArchitectState:
        ...
        try:
            ...
            agent = Agent(
                role="refactoring_architect Agent",
                goal="You exist to solve the dangerous refactor problem. Without safety:
- Hidden dependencies cause break",
                backstory="""You are the refactoring_architect Agent, a specialized code transformation expert powered by floyd-safe-ops and floyd-patch technologies. You plan, simulate, and execute refactoring operations with maximum safety.
...
- You want to verify correctness after refactoring
    ...
    @listen("execute")
    async def finalize(self, state: RefactoringArchitectState) -> RefactoringArchitectState:
