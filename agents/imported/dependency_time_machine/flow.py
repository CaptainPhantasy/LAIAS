...
class DependencyTimeMachineState(BaseModel):
    """Typed state for dependency_time_machine Agent."""
...
class DependencyTimeMachineFlow(Flow[DependencyTimeMachineState]):
    ...
    def _initialize_tools(self) -> List:
        ...
        for tool_name in ['FileReadTool', 'DirectoryReadTool']:
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> DependencyTimeMachineState:
    ...
    @listen("initialize")
    async def execute(self, state: DependencyTimeMachineState) -> DependencyTimeMachineState:
        ...
        try:
            ...
            agent = Agent(
                role="dependency_time_machine Agent",
                goal="You exist to solve the dependency surprise problem. Without future awareness:
- Breaking changes cat",
                backstory="""You are the dependency_time_machine Agent, a specialized trend forecaster responsible for predicting the future health and trajectory of dependencies.
...
- You need to create a dependency roadmap
    ...
    @listen("execute")
    async def finalize(self, state: DependencyTimeMachineState) -> DependencyTimeMachineState:
...
async def main():
    ...
    flow = DependencyTimeMachineFlow()
