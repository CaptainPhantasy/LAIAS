...
class DiagnosticSurgeonState(BaseModel):
    """Typed state for diagnostic_surgeon Agent."""
...
class DiagnosticSurgeonFlow(Flow[DiagnosticSurgeonState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> DiagnosticSurgeonState:
    ...
    @listen("initialize")
    async def execute(self, state: DiagnosticSurgeonState) -> DiagnosticSurgeonState:
        ...
        try:
            ...
            agent = Agent(
                role="diagnostic_surgeon Agent",
                goal="You exist to solve the mysterious failure problem. Without systematic diagnosis:
- Symptoms are trea",
                backstory="""You are the diagnostic_surgeon Agent, a specialized system troubleshooter powered by floyd-devtools technology. You diagnose complex build failures, test failures, dependency issues, and performance problems with surgical precision.
...
- You need systematic debugging beyond simple error reading
    ...
    @listen("execute")
    async def finalize(self, state: DiagnosticSurgeonState) -> DiagnosticSurgeonState:
