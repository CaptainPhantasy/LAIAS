...
class StagnationInterceptorState(BaseModel):
    """Typed state for stagnation_interceptor Agent."""
...
class StagnationInterceptorFlow(Flow[StagnationInterceptorState]):
    ...
    @start()
    async def initialize(self, inputs: Dict[str, Any]) -> StagnationInterceptorState:
    ...
    @listen("initialize")
    async def execute(self, state: StagnationInterceptorState) -> StagnationInterceptorState:
        ...
        try:
            ...
            agent = Agent(
                role="stagnation_interceptor Agent",
                goal="You exist to solve the solo developer's curse: without a team to provide external perspective, it's ",
                backstory="""You are the stagnation_interceptor Agent, a specialized productivity coach with a single mission: keep development moving forward and prevent endless rabbit holes.
...
- You're prone to yak shaving and need external discipline
    ...
    @listen("execute")
    async def finalize(self, state: StagnationInterceptorState) -> StagnationInterceptorState:
...
async def main():
    ...
    flow = StagnationInterceptorFlow()
